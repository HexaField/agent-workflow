import {
  ...
  runAgentWorkflow,
  RunMeta,
  researchVerifierWorkflowDefinition
...
describe('Researcher/verifier collaboration loop', () => {
  opencodeTestHooks()

  it('completes a simple research synthesis task', async () => {
    ...
    const scenario = `Create a readme.md file that summarizes three reputable sources about the benefits of green tea and include the phrase "Green tea benefits".`

    const response = await runAgentWorkflow(researchVerifierWorkflowDefinition, {
    ...
    const workerStep = firstRound.steps.researcher
    ...
    const namespacedWorkerName = `${researchVerifierWorkflowDefinition.id}.researcher`
    const namespacedVerifierName = `${researchVerifierWorkflowDefinition.id}.verifier`
    ...
    for (const entry of logs) {
      ...
      expect(
        userMessages.some(
          (message) =>
            typeof message.payload?.instructions === 'string' && message.payload.instructions.includes('Green tea benefits')
        )
      ).toBe(true)
    }
    ...
    expect(readmeContent.includes('Green tea benefits')).toBe(true)
    ...
    expect(readmeDiff?.after.toLowerCase()).toContain('green tea benefits')
